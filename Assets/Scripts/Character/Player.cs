...
public class Player : Character
{
    ...
    private void Update()
    {
        ...
        if (timer <= 0)
        {
            ...
            Energy += getEnergy;
            if(Timer.GetInstance.timer>=3)Energy -= 4;
    ...
    private void FixedUpdate()
    {
        ...
        if(rb.angularVelocity<-maxAngle) rb.angularVelocity =- maxAngle;
        if(transform.position.x>335)UIManager.GetInstance.EnterPanel(WinScene.GetInstance.gameObject);  
    }
    /*public static Player GetInstance()
    {
        if (player == null)
    ...
    public int Energy
    {
        set 
        {
            ...
            if (energy <= 0) 
            {
                ...
                UIManager.GetInstance.EnterPanel(EndScene.GetInstance.gameObject);
             }
        }
        get { return energy; }
    }
